...
#include "XmsgChannelStatusSub.h"

XmsgChannelStatusSub::XmsgChannelStatusSub()
{

}

void XmsgChannelStatusSub::handle(shared_ptr<XmsgNeUsr> nu, const string& ap, shared_ptr<XmsgClient> client, SptrXitp trans, shared_ptr<XmsgChannelStatusSubReq> req)
{
	if (req->cgt().empty())
	{
	...
	ChannelStatusMgr::instance()->future([ap, client, trans, req]
	{
		ChannelStatusMgr::instance()->getSubPub()->sub(ap, client->ccid, req);
		shared_ptr<XmsgChannelStatusSubRsp> rsp(new XmsgChannelStatusSubRsp());
		for (int i = 0; i < req->cgt_size(); ++i)
		{
			XmsgChannelStatusSub::query4usr(req->cgt(i), rsp);
		}
...
void XmsgChannelStatusSub::query4usr(const string& cgt, shared_ptr<XmsgChannelStatusSubRsp> rsp)
{
