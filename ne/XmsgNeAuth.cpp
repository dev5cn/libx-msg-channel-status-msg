...
void XmsgNeAuth::handle(shared_ptr<XscChannel> channel, SptrXitp trans, shared_ptr<XmsgNeAuthReq> req)
{
	...
	rsp->set_cgt(XmsgChannelStatusCfg::instance()->cfgPb->cgt());
	trans->end(rsp);
	if (old == nullptr)
		return;
...
shared_ptr<XmsgChannelStatusCfgXmsgNeN2hAddr> XmsgNeAuth::findXmsgNeCfg(const string& cgt)
{
	for (int i = 0; i < XmsgChannelStatusCfg::instance()->cfgPb->n2h_size(); ++i)
	{
		auto ne = XmsgChannelStatusCfg::instance()->cfgPb->n2h(i);
		if (ne.cgt() != cgt)
			continue;
		shared_ptr<XmsgChannelStatusCfgXmsgNeN2hAddr> pb(new XmsgChannelStatusCfgXmsgNeN2hAddr());
		pb->CopyFrom(ne);
		return pb;
	}
	return nullptr;
